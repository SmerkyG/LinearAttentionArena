...
class RWKV_Tmix_x060g(MyModule):
    def __init__(self, args, layer_id):
        ...
        self.dim_ffn = int(args.n_embd * 2.5) // 32 * 32
        ...
        self.v_ffn_biggate = nn.Linear(args.n_embd, args.dim_att + self.dim_ffn + args.dim_att + self.dim_ffn, bias=False) # 2D(V+F) params
        self.output = nn.Linear(args.dim_att + self.dim_ffn, args.n_embd, bias=False) # D(V+F) params
    ...
    def forward(self, x):
        ...
        vffn, biggate = self.v_ffn_biggate(v).split([self.dim_att+self.dim_ffn, self.dim_att+self.dim_ffn], dim=-1)
        vffn = vffn * nn.functional.silu(biggate)
        v, ffn = vffn.split([self.dim_att, self.dim_ffn], dim=-1)
        v = v.contiguous()
