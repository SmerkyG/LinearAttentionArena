...
class CMoE_b(nn.Module):
    ...
    def __init__(self, args:Transformer_Config, layer_id):
        ...
        if args.num_experts > 1:
            self.moe = MoE(input_size=args.n_embd*2, expert=RWKV_Expert(args, layer_id), num_experts = args.num_experts, ep_size=args.ep_size, k=1, min_capacity=4, capacity_factor=1, eval_capacity_factor=1, drop_tokens=True, hash_prime=hash_prime)
    ...
    def forward(self, x:Tensor, token_ids:Tensor, last_state:ChannelMixState):
        ...
        xk_xr = torch.concat([xk, xr], dim=-1)
        if self.num_experts > 1:
            return self.moe(xk_xr, token_ids, used_token=torch.tensor([])), ChannelMixState(x[:, -1])
        else:
            return self.moe(xk_xr), ChannelMixState(x[:, -1])
...
class RWKV_Expert(nn.Module):
    ...
    def forward(self, xk_xr):
        xk, xr = xk_xr.chunk(2, dim=-1)
        return torch.sigmoid(self.ffn_receptance(xr)) * self.ffn_value( torch.relu( self.ffn_key(xk) ).square() )
