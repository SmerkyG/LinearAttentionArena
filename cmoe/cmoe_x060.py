...
class CMoE_x060(nn.Module):
    def __init__(self, args, layer_id):
        ...
        dim_ffn = args.n_embd * 16
        self.key = nn.Linear(args.n_embd, dim_ffn, bias=False)
        self.receptance = nn.Linear(args.n_embd, args.n_embd, bias=False)
        self.value = nn.Linear(dim_ffn, args.n_embd, bias=False)
        nn.init.orthogonal_(self.ffn_key.weight)
        nn.init.zeros_(self.ffn_receptance.weight)
        nn.init.zeros_(self.ffn_value.weight)

    def forward(self, x, token_ids, last_state:ChannelMixState):
